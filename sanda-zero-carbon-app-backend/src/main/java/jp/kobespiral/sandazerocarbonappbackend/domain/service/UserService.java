...
public class UserService {
    ...
    public UserDailyStatus renewUserDailyStatusForQuiz(String userId,Long answeredQuizId){
        AnsweredQuiz answeredQuiz =  answeredQuizRepository.findById(answeredQuizId).orElseThrow(()->new UserValidationException(USER_DOES_NOT_EXIST,"create the user", String.format("crate %d",userId)));
        //日付タイプの変更
        ...
        Quiz quiz = quizRepository.findById(answeredQuiz.getQuizId()).orElseThrow(()->new UserValidationException(USER_DOES_NOT_EXIST,"create the user", String.format("crate %d",userId)));
        UserDailyStatus userDailyStatus = userDailyStatusRepository.findByUserIdAndDate(userId, localDate);
