...
public class TotalConditionService {
    ...
    public void calculateTotalCondition() {
        ...
        LocalDateTime now; // 今の時間
        LocalDateTime now_cut; // 今の時間の切り捨て
        now = LocalDateTime.now();
        now_cut = LocalDateTime.now().withMinute(0).withSecond(0).withNano(0); // 現在時刻の分以下切り捨てを指定

        if (totalCondition == null) { // 市の状況が存在しない場合
            // 参考
            // https://qiita.com/swordone/items/5fbf5ed138d812e117ec
            since = now_cut.minusHours(INTERVAL); // 生成する市の状況のいつからをいつまでの1時間前に指定
            until = now_cut;
        } else { // 市の状況が存在する場合
        ...
        while(now.isAfter(until)){
            ...
            since = since.plusHours(INTERVAL);
            until = until.plusHours(INTERVAL);
        }
//
    }
